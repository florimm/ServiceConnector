...
namespace FakeBW
{
    ...
    public class BrowserResponseBodyWrapper : IEnumerable<byte>
    {
        ...
        public BrowserResponseBodyWrapper(HttpResponse response)
        {
            var contentStream =
        ...
        private static MemoryStream GetContentStream(HttpResponse response)
        {
            var contentsStream = new MemoryStream();
            response.BinaryWrite(contentsStream.ToArray());
